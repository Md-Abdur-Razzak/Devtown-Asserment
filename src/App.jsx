...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://dev-town-server-ten.vercel.app/serching`);
  ...
  return (
    <div>
        ...
        <form onSubmit={handleSearch}>
          <div className="join">
            <input
              ...
              name="searchTerm"
            />
            <button className="btn join-item rounded-r-full text-2xl">
              <FaSearch></FaSearch>
            </button>
          </div>
        </form>
      ...
      {
          searchResults.length==0 && <div className="flex justify-center"><ItemEmpty></ItemEmpty></div>
      }
      <div className="xl:w-[70%] lg:w-[80%] md:w-[90%] mx-auto grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1  max-[540px]:w-[80%] w-[60%] gap-4 mt-9">
        ...
        {searchResults.map((item, index) => (
          <div
            key={index}
            className="border flex flex-col border-t-4 rounded-md border-[#4e4ef4] shadow-lg "
          >
